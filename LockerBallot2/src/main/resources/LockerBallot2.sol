...
contract MultiSigWallet {
       ...
       validRequirement(_owners.length, _required)
   {
    ...
    /// @dev 审批员发起审批，并自动投赞成票.
    function submitAgreeTransaction(uint transactionId,address[] _owners,uint _required)
        public
        ownerExists(msg.sender)
        returns (uint _transactionId)
    {
        _transactionId = addTransaction(transactionId,_owners,_required);
        confirmTransaction(transactionId);
    }

     /// @dev 审批员发起审批，并自动投反对票.(暂时没用)
    function submitRefuseTransaction(uint transactionId,address[] _owners,uint _required)
        public
        ownerExists(msg.sender)
        returns (uint _transactionId)
    {
        _transactionId = addTransaction(transactionId,_owners,_required);
        refuseConfirmation(transactionId);
    }
     ...
     * Internal functions
     */
    /// @dev locker的员工发起审批流程，用此方法
    /// @param transactionId Transaction transactionId.
    /// @return Returns transaction ID.
    function addTransaction(uint transactionId,address[] _owners,uint _required)
        public
