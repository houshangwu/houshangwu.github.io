...
contract MultiSigWallet {
        ...
        validRequirement(_owners.length, _required)
    {
        ...
        for (uint i=0; i<_owners.length; i++) {
            require(!isStaff[_owners[i]] && !isOwner[_owners[i]] && _owners[i] != 0);
    ...
    /// @dev 审批员发起审批，并自动投赞成票.
    function submitAgreeTransaction(uint transactionId)
        public
        ownerExists(msg.sender)
        returns (uint _transactionId)
    {
        _transactionId = addTransaction(transactionId);
        confirmTransaction(transactionId);
    }

     /// @dev 审批员发起审批，并自动投反对票.(暂时没用)
    function submitRefuseTransaction(uint transactionId)
        public
        ownerExists(msg.sender)
        returns (uint _transactionId)
    {
        ...
        returns (bool)
    {
        uint count = 0;
        for (uint i=0; i<owners.length; i++) {
            if (confirmations[transactionId][owners[i]] == 1)
                count += 1;
            if (count >= required)
                return true;
        }
